...
export default function Overview({
    ...
    selectedYear?: ConfirmedYearStats | null;
    ...
    return (
        <div className="grid h-full min-h-0 grid-rows-[1fr_1.05fr] gap-4">
            <div className="grid min-h-0 gap-4 md:grid-cols-2 xl:grid-cols-4">
                <StatCard label="Total Games" value={num(stats.library_games)} detail="Library game entries" delta={metricGrowth('library_games', stats, baseline)} icon={Gamepad2} />
                <StatCard label="Completed Games" value={num(stats.completed)} detail="Completed + 100%" delta={metricGrowth('completed', stats, baseline)} icon={Trophy} />
                <StatCard label="100% Games" value={num(stats.hundred_percent)} detail="Perfect achievement runs" delta={metricGrowth('hundred_percent', stats, baseline)} icon={Trophy} />
                <StatCard label="Playtime" value={hours(stats.playtime_hours)} detail="Total tracked hours" delta={metricGrowth('playtime_hours', stats, baseline)} icon={Clock3} />
            </div>
            <div className="grid min-h-0 gap-4 xl:grid-cols-[1.2fr_0.8fr_0.8fr]">
                <article className="min-h-0 rounded-[26px] border border-black/10 bg-black p-5 text-white shadow-[0_20px_55px_rgb(9_14_12/0.14)]">
                    ...
                    <div className="mt-4 flex items-end justify-between gap-4">
                        ...
                        <PercentDeltaBadge value={metricGrowth('achievement_progress', stats, baseline)} />
                    </div>
                ...
                <StatCard label="Base Value" value={money(stats.base_value)} detail="Digital/Physical + owned DLCs" delta={metricGrowth('base_value', stats, baseline)} icon={DollarSign} />
                <StatCard label="Paid Value" value={money(stats.purchased_value)} detail="What you paid" delta={metricGrowth('purchased_value', stats, baseline)} icon={DollarSign} />
            </div>
        </div>
    );
}
